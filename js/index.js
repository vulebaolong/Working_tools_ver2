...
`
let html_Erorr = `
    <div class="item_erorr button_long none">
        <label>erorr</label>
        <div class="menu_erorr">
            <div class="item_erorr1 button_long"><label>erorr1</label></div>
            <div class="item_erorr2 button_long"><label>erorr2</label></div>
            <div class="item_erorr3 button_long"><label>erorr3</label></div>                                                              
        </div>
    </div>
`
...
window.onload = function () {
    ...
    const sheetName_erorr = 'Request_email_erorr'
    const url_erorr = `https://docs.google.com/spreadsheets/d/${sheetID}/gviz/tq?&sheet=${sheetName_erorr}`
    ...
    Authen_Erorr(url_Authen, 'A', 1)
        .then((datass) => {
            return Authen_Erorr(url_Authen, 'A', 2, datass)
        })
        .then((e) => {
            console.log('url_Authen: ', e);
            if (e) {

                //Kiểm tra sử dụng Create_Erorr
                Authen_Erorr(url_erorr, 'A', 1)
                    .then((datass) => {
                        return Authen_Erorr(url_erorr, 'A', 2, datass)
                    })
                    .then((e) => {
                        console.log('url_erorr: ', e);
                        START(S_inhouse, S_order, e)
                    })
                //===================================   
            } else {
                console.log('Người dừng chưa đăng ký sử dụng');
            }
        })
...
function START(S_inhouse, S_order, Authen) {
    ...
    if (S_inhouse != -1 && S_order == -1) {
        ...
        if (root.children.length > 0) {
            var el_main_container = root.children[0].children[0].children[1]
            console.log(el_main_container);
            console.log(el_main_container.children.length);
            if (el_main_container.children.length > 0) {
